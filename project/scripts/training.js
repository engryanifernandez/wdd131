...
const displayExistingBookings = () => {
    ...
    bookings.forEach(booking => {
        ...
        row.innerHTML = `
            <td data-label="Name">${booking.traineeName}</td>  
            <td data-label="Session">${booking.sessionType}</td>
            <td data-label="Date">${booking.date}</td>
            <td data-label="Time">${booking.time}</td>
            <td data-label="Duration">${booking.duration} hours</td>
            <td data-label="Instructor">${booking.instructor}</td>
          `;
...
const isBookingAvailable = newBooking => {
    ...
    return !bookings.some(booking => {
        if (booking.sessionType !== newBooking.sessionType) return false;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const bookingForm = document.getElementById("trainingForm");
    if (bookingForm) {
